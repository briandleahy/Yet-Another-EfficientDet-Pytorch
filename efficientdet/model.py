...
class SeparableConvBlock(nn.Module):
    ...
    def __init__(
            self, in_channels, out_channels=None, norm=True, activation=False,
            onnx_export=False):
        ...
        # A: Confirmed, just pointwise_conv applies bias, depthwise_conv
        #  has no bias.
...
class BiFPN(nn.Module):
    ...
    def __init__(
            self, num_channels, conv_channels, first_time=False, epsilon=1e-4,
            onnx_export=False, attention=True, use_p8=False):
        """

        Args:
            num_channels:
            conv_channels:
            first_time: whether the input comes directly from the
                        efficientnet, if True, downchannel it first, and
                        downsample P5 to generate P6 then P7
            epsilon: epsilon of fast weighted attention sum of BiFPN,
                        not the BN's epsilon
    ...
    def forward(self, inputs):
        ...
        # downsample channels using same-padding conv2d to target
        # phase's if not the same
        # judge: same phase as target,
        # if same, pass;
        # elif earlier phase, downsample to target phase's by pooling
        # elif later phase, upsample to target phase's by nearest
        # interpolation
...
class Regressor(nn.Module):
    ...
    def __init__(
            self, in_channels, num_anchors, num_layers, pyramid_levels=5,
            onnx_export=False):
    ...
    def forward(self, inputs):
        ...
        for feat, bn_list in zip(inputs, self.bn_list):
            for i, (bn, conv) in enumerate(zip(bn_list, self.conv_list)):
                feat = conv(feat)
                feat = bn(feat)
...
class Classifier(nn.Module):
    ...
    def __init__(
            self, in_channels, num_anchors, num_classes, num_layers,
    ...
    def forward(self, inputs):
        ...
        for feat, bn_list in zip(inputs, self.bn_list):
            ...
            feat = feat.contiguous().view(
                feat.shape[0], feat.shape[1], feat.shape[2], self.num_anchors,
                self.num_classes,
            )
...
class EfficientNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # TODO: temporarily storing extra tensor last_x and del it later
        # might not be a good idea, try recording stride changing when
        # creating efficientnet, and then apply it here.
        last_x = None
