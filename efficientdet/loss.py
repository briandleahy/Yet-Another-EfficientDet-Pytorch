...
def calc_iou(a, b):
    ...
    iw = (
        torch.min(torch.unsqueeze(a[:, 3], dim=1), b[:, 2]) -
        torch.max(torch.unsqueeze(a[:, 1], dim=1), b[:, 0])
    )
    ih = (
        torch.min(torch.unsqueeze(a[:, 2], dim=1), b[:, 3]) -
        torch.max(torch.unsqueeze(a[:, 0], dim=1), b[:, 1])
    )
...
class FocalLoss(nn.Module):
    ...
    def forward(
        ...
        for j in range(batch_size):
            ...
            this_classification_loss = (
                cls_loss.sum() /
                torch.clamp(num_positive_anchors.to(dtype), min=1.0)
            )
            classification_losses.append(this_classification_loss)
            ...
            if positive_indices.sum() > 0:
                ...
                gt_widths = (
                    assigned_annotations[:, 2] -
                    assigned_annotations[:, 0]
                )
                gt_heights = (
                    assigned_annotations[:, 3] -
